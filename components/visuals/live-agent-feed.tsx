...
const AGENT_NAMES = [
  "analytics-7f2a", "sync-3b1c", "monitor-9d4e", "reporting-2c8f",
  "scheduler-5a3d", "billing-1e9b", "audit-4f2c", "pipeline-8d1a",
  "review-6b3e", "ingest-0c7f",
]
...
function ProgressBar({ initial }: { initial: number }) {
  ...
  return (
    <div style={{ width: "100%", height: 2, background: "rgb(var(--ink-rgb) / 0.08)", borderRadius: 9 }}>
      <div style={{
...
const SEED_ROWS: AgentRow[] = [
  { id: "A1B2C3", name: "reporting-7f2a", task: "Generating Q2 financial report",       region: "us-east",    status: STATUSES[0], progress: 42, elapsed: "3m 12s", key: 0 },
  { id: "D4E5F6", name: "sync-3b1c",      task: "Running integration test suite",       region: "eu-west",    status: STATUSES[0], progress: 67, elapsed: "7m 48s", key: 1 },
  { id: "G7H8I9", name: "analytics-2c8f", task: "Aggregating competitor pricing data",  region: "us-west",    status: STATUSES[3], progress: 18, elapsed: "1m 05s", key: 2 },
  { id: "J0K1L2", name: "scheduler-5a3d", task: "Syncing Notion docs with Linear",      region: "eu-central", status: STATUSES[0], progress: 55, elapsed: "5m 30s", key: 3 },
  { id: "M3N4O5", name: "pipeline-8d1a",  task: "Migrating data platform — 3 tables",   region: "ap-south",   status: STATUSES[0], progress: 80, elapsed: "11m 22s", key: 4 },
    ...
    <div style={{
      ...
      <div style={{
        ...
        background: "rgb(var(--ink-rgb) / 0.03)",
          ...
          {["SERVICE", "TASK", "REGION", "STATUS"].map(h => (
          <span key={h} style={{ fontSize: 8, letterSpacing: "0.16em", color: "rgb(var(--ink-rgb) / 0.30)", fontFamily: "monospace" }}>{h}</span>
        ))}
      </div>
        ...
        {rows.map((row, i) => (
          <div
            ...
            style={{
              ...
              gridTemplateColumns: "80px 1fr 80px 70px",
              padding: "10px 16px",
              borderBottom: "1px solid rgb(var(--ink-rgb) / 0.04)",
              gap: 8,
              alignItems: "center",
          ...
          >
            {/* Agent */}
            <div>
              <div style={{ fontSize: 9, fontFamily: "monospace", color: "rgb(var(--ink-rgb) / 0.65)", marginBottom: 1 }}>{row.name}</div>
              <div style={{ fontSize: 7.5, fontFamily: "monospace", color: "rgb(var(--ink-rgb) / 0.25)" }}>#{row.id}</div>
            </div>

            {/* Task + progress */}
            <div style={{ minWidth: 0 }}>
              <div style={{
                fontSize: 9, color: "rgb(var(--ink-rgb) / 0.50)", lineHeight: 1.35, marginBottom: 5,
            ...
            {/* Region */}
            <div style={{ fontSize: 8, fontFamily: "monospace", color: "rgb(var(--ink-rgb) / 0.30)" }}>{row.region}</div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              ...
              <span style={{ fontSize: 8, fontFamily: "monospace", color: "rgb(var(--ink-rgb) / 0.35)" }}>{row.status.label}</span>
            </div>
          </div>
        ))}
      </div>
