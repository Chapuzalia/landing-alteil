...
const AGENTS = [
  {
    ...
    desc: "Connects your systems, centralizes operational data, and synthesizes reports from multiple sources with a single source of truth.",
    stats: [{ v: "2.4M", l: "records synced" }, { v: "98.2%", l: "accuracy" }],
    img: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/researcher-CvhqOuV6irGwBOnJoTGFlXdbyYBRjb.png",
  },
  {
    label: "INTEGRATIONS",
    title: "System integration",
    desc: "Connects apps, APIs, and databases across 40+ platforms. Keeps data in sync, resolves conflicts, and automates handoffs.",
    stats: [{ v: "1.1M", l: "syncs run" }, { v: "3.2s", l: "avg latency" }],
  ...
  },
  {
    ...
    desc: "Takes action across your stack: sends messages, creates events, triggers webhooks, and orchestrates third-party apps.",
              ...
              <div className="group relative bg-card rounded-2xl border border-foreground/[0.07] overflow-hidden cursor-pointer">
                ...
                {agent.img && (
                  <div className="hidden md:block absolute inset-y-0 right-0 w-1/2 pointer-events-none">
                    ...
                    <div
                      ...
                      style={{
                        background: "linear-gradient(to right, var(--card) 0%, transparent 55%)",
                      }}
                    />
                  </div>
                )}
                ...
                >
                  <div className="md:max-w-[60%]">
                    ...
                    <p className="text-sm text-foreground/45 leading-relaxed mb-8">{agent.desc}</p>
                  </div>
                  <div className="flex gap-8 pt-6 border-t border-foreground/[0.06]">
                    {agent.stats.map(s => (
                      <div key={s.l}>
                        <div className="text-2xl font-light">{s.v}</div>
                        <div className="text-[11px] text-foreground/35 tracking-widest mt-0.5">{s.l}</div>
                      </div>
                    ))}
                  </div>
                </div>
