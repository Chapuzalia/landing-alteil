...
export function TeamSection({ team }: { team: TeamContent }) {
  ...
  return (
    <section id="team" className="py-32 px-6 md:px-12 lg:px-20 border-t border-foreground/6">
            ...
            {team.cards.map((card, i) => (
              <BentoCard key={card.title} className="p-7" delay={i * 80}>
                <div className="flex gap-5">
                  <div className="mt-1 h-10 w-10 shrink-0 rounded-xl border border-foreground/10 bg-foreground/3 flex items-center justify-center text-xs text-foreground/35 tracking-widest">0{i + 1}</div>
                  <div>
